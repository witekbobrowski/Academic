    ...
    getConnection((err, connection) => {
        ...
        connection.query('SELECT * FROM students WHERE MATCH(firstName, lastName, contactName, contactEmail, homeTown) AGAINST (?)', [phrase], (error, rows, fields) => {
            ...
            for (row in rows) {
                ...
                output.push('<td type="button" id="button-student-info" data-id="' + rows[row].student_ID + '" data-modal="student-info">' + rows[row].birthDate.getDay() + "/" + rows[row].birthDate.getMonth() + "/" + rows[row].birthDate.getFullYear() + '</td>')
